...
#define CMAKE_ROOT_DIR(name) (string(string(ROOT_DIR) + name))
    ...
    public:
        ...
        Config(const string &cfg_path, const string &name_space = "rog_map") {
            ...
            /// Input cloud preprocessing before mapping.
            loader.LoadParam(name_space + "/ros_callback/body_crop_en", body_crop_en, false);
            vector<double> temp_body_crop_box;
            loader.LoadParam(name_space + "/ros_callback/body_crop_box", temp_body_crop_box,
                             vector<double>{0, 0, 0});
            if (body_crop_en) {
                if (temp_body_crop_box.size() != 3) {
                    throw std::invalid_argument("Body crop box size is not 3!");
                }
                body_crop_half_box = Vec3f(temp_body_crop_box[0], temp_body_crop_box[1], temp_body_crop_box[2]);
            }
            loader.LoadParam(name_space + "/ros_callback/voxel_leaf_size", voxel_leaf_size, -1.0);
...
#define logit(x) (log((x) / (1 - (x))))
        ...
        /* Input cloud preprocessing: crop the drone body and downsample the cloud */
        bool body_crop_en{false};
        Vec3f body_crop_half_box{};
