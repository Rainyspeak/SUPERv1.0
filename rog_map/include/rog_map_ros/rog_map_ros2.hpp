...
namespace rog_map {
    ...
    class ROGMapROS : public ROGMap {
        ...
        void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr cloud_msg) {
            ...
            /// Preprocess the world-frame input cloud before mapping:
            ///  1) remove NaN points;
            ///  2) crop the drone body around the odom (ROG-Map has no body filter,
            ///     without this the drone itself would become an obstacle);
            ///  3) voxel downsample to reduce the mapping load.
            if (cfg_.body_crop_en || cfg_.voxel_leaf_size > 0) {
                pcl::PointCloud<PclPoint>::Ptr pc_in = temp_pc.makeShared();
                std::vector<int> nan_idx;
                pcl::removeNaNFromPointCloud(*pc_in, *pc_in, nan_idx);
                if (cfg_.body_crop_en && !pc_in->empty()) {
                    pcl::PointCloud<PclPoint>::Ptr pc_body(new pcl::PointCloud<PclPoint>());
                    pcl::CropBox<PclPoint> body_crop;
                    body_crop.setMin(Eigen::Vector4f(
                            robot_state_.p.x() - cfg_.body_crop_half_box.x(),
                            robot_state_.p.y() - cfg_.body_crop_half_box.y(),
                            robot_state_.p.z() - cfg_.body_crop_half_box.z(), 1.0f));
                    body_crop.setMax(Eigen::Vector4f(
                            robot_state_.p.x() + cfg_.body_crop_half_box.x(),
                            robot_state_.p.y() + cfg_.body_crop_half_box.y(),
                            robot_state_.p.z() + cfg_.body_crop_half_box.z(), 1.0f));
                    body_crop.setNegative(true);
                    body_crop.setInputCloud(pc_in);
                    body_crop.filter(*pc_body);
                    pc_in = pc_body;
                }
                if (cfg_.voxel_leaf_size > 0 && !pc_in->empty()) {
                    pcl::PointCloud<PclPoint>::Ptr pc_down(new pcl::PointCloud<PclPoint>());
                    pcl::VoxelGrid<PclPoint> voxel;
                    const float leaf = static_cast<float>(cfg_.voxel_leaf_size);
                    voxel.setLeafSize(leaf, leaf, leaf);
                    voxel.setInputCloud(pc_in);
                    voxel.filter(*pc_down);
                    pc_in = pc_down;
                }
                temp_pc = *pc_in;
            }
